...
fn run_mapper_worker_std(
    ...
    if enable_profiling {
        ...
        let p_regex = if total_cycles > 0 {
            (c_regex as f64 / total_cycles as f64) * 100.0
        } else {
            0.0
        };
        let p_parse = if total_cycles > 0 {
            (c_parse as f64 / total_cycles as f64) * 100.0
        } else {
            0.0
        };
        let p_map = if total_cycles > 0 {
            (c_map as f64 / total_cycles as f64) * 100.0
        } else {
            0.0
        };

        println!(
            "Thread Profile (std): Regex={:.1}% Parse={:.1}% Map={:.1}% (Total Cycles: {})",
...
fn run_mapper_worker_pcre(
    ...
    if enable_profiling {
        ...
        println!(
            "Thread Profile (pcre): Regex={:.1}% Parse={:.1}% Map={:.1}% (Total Cycles: {})",
            p_regex, p_parse, p_map, total_cycles
        );
    }
